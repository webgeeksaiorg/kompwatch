...
When you add a competitor to CompeteWatch, you can optionally specify a **CSS selector**. This tells our scraper exactly which part of the page to monitor, rather than watching the entire page.
...
If you leave the selector field blank, CompeteWatch defaults to `body` — meaning it watches the entire page. This works but generates more noise.
...
- **Test in your browser console.** Open DevTools, go to the Console tab, and type `document.querySelector('YOUR_SELECTOR')` to verify it finds the right element before saving it in CompeteWatch.
...
If a competitor redesigns their site, your selector may no longer match any element. CompeteWatch will flag this as a monitoring error. Go to your competitor's settings and update the selector.
