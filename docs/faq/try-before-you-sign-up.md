# Try KompWatch Before You Sign Up

There are three ways to see KompWatch without creating an account:

1. **60-second video** — on the [homepage](https://kompwatch.com), scroll to "See it in action" and click Play. Captions are included; no audio required.
2. **Interactive demo** — explore the full product at [kompwatch.com/demo](https://kompwatch.com/demo) — no account, no credit card, no email required.
3. **Public changelog** — see everything we've shipped at [kompwatch.com/changelog](https://kompwatch.com/changelog) — a timeline-style history of features and improvements.

---

You can explore a fully interactive demo of KompWatch at [kompwatch.com/demo](https://kompwatch.com/demo) — no account, no credit card, no email required.

## What's in the Demo

The demo loads a live sample dashboard with four pre-seeded competitors and real-looking data:

- A full **Changes Timeline** with sample CRITICAL, HIGH, MEDIUM, and LOW severity detections — click any row to expand it and see the AI factual context inline
- The **Activity Heatmap** — the 26-week GitHub-style grid showing change volume over time
- **Competitor detail pages** for each sample competitor, with snapshot history and AI-generated "Why this matters" context
- **Expandable change rows** — click any change in the timeline to reveal the full AI-generated factual diff ("What changed") alongside the strategic context
- **Digest preview** — the same email format you'd receive as a paying customer

All data is read-only sample data. Nothing is scraped in real time and no real competitors are added.

## How Is the Demo Different from a Real Account?

| | /demo | Free account |
|---|---|---|
| Sign-up required | No | Yes (magic link) |
| Real competitor monitoring | No | Yes (up to 2) |
| Add your own URLs | No | Yes |
| Receive digest emails | No | Yes |
| Data retention | None | Stored in your account |

The demo is a preview only. To start tracking your actual competitors, [create a free account →](https://kompwatch.com/login).

## Can I Share the Demo with a Teammate?

Yes — the `/demo` URL is public and shareable. It's useful for getting buy-in before signing up: paste the link in Slack, a proposal doc, or a sales call.

## The Demo Looks Different from What I Expected

The demo uses fixed sample data and may not reflect the exact layout of your real dashboard (which adapts based on your plan and competitor count). If something looks off or you have questions about a specific feature, email [support@kompwatch.com](mailto:support@kompwatch.com).

---
*Ready to monitor your real competitors? [Start free →](https://kompwatch.com/login)*
